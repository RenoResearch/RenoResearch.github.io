...
_flutter.buildConfig = {"engineRevision":"77e2e94772b6eb43759e34ed1ad7da4674e19cab","builds":[{"compileTarget":"dart2js","renderer":"canvaskit","mainJsPath":"main.dart.js"},{}]};
 and "2567378696" /* Flutter's service worker is deprecated and will be removed in a future Flutter release. */
// are substituted by the build. Keep them intact.
//
// WHY THIS EXISTS
// ---------------
// The app ships the CanvasKit renderer, which paints the whole UI onto a WebGL
// surface. Embedded in-app browsers (WhatsApp, Instagram, Facebook/Messenger,
// TikTok, etc.) run inside an Android System WebView whose GPU/WebGL compositing
// is buggy. CanvasKit then renders corrupted solid-colour / blank blocks instead
// of the real content.
//
// Fix: for those environments only, force CanvasKit's CPU (software) renderer via
// `canvasKitForceCpuOnly`. It bypasses WebGL entirely — a little slower, but it
// renders correctly. Normal browsers keep the fast GPU path untouched.
...
(function () {
  ...
  // Known in-app browsers whose embedded WebViews break WebGL/CanvasKit.
  ...
  // Fallback: WebGL is entirely unavailable in this browser. CanvasKit cannot use
  // the GPU path at all here, so software rendering is the only way it will paint.
  var webGLUnavailable = (function () {
    try {
  ...
  var config = {};
  if (isKnownInAppBrowser || webGLUnavailable) {
    // Software rasterisation — reliable inside broken in-app WebViews.
    config.canvasKitForceCpuOnly = true;
  }

  _flutter.loader.load({
    serviceWorkerSettings: {
      serviceWorkerVersion: "2567378696" /* Flutter's service worker is deprecated and will be removed in a future Flutter release. */,
    },
    config: config,
  });
})();
